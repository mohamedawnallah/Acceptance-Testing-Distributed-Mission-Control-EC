...
# Function to import mission control data.
function import_mc_data() {
  local node=$1
...
function make_payments() {
  ...
  if [ $PATH_LENGTH -gt 2 ]; then
    ...
    source_destination=$(echo "$source_destination" | jq --argjson mc_data "$mc_data" '. + {mc_data: $mc_data}' | jq --argjson payment_attempts_before_mc "$payment_response_after_failing_last_edge" '. + {payment_attempts_before_mc: $payment_attempts_before_mc}')
...
# Read the source-destination nodes file using jq.
